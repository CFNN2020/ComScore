...
df_sites_uk = pd.read_sql_query('select * from uk_comscore_site_rankings', con=work_engine)
uk_feb = pd.read_sql_query('select * from uk_site_rankings_feb', con=work_engine)
joined_engagement = pd.read_sql_query('select * from uk_joined_site_rankings', con=work_engine)

fig1 = go.Figure()

x = df_sites_uk['media_property']
x2 = uk_feb['media_property']
y = df_sites_uk['total_uv_in_thousands']
y2 = uk_feb['total_uv_in_thousands']
...
fig3 = px.scatter(df_sites_uk, y='avg_mins_per_visit', x='avg_mins_per_visitor', color='media_property')
fig3.update_layout(paper_bgcolor="#acc4cc")
fig4 = px.scatter(df_sites_uk, y='avg_views_per_visit', x='avg_mins_per_visit', color='media_property')
fig4.update_layout(paper_bgcolor="#acc4cc")

layout = html.Div(children=[
    html.Div([
        html.H1(children='PCGamesN and competitor sites by UV'), 
        html.H2(children='UK site visitors and views include key competitor sites and PCGN', style={'fontSize':14}),
        dcc.Graph(
            id='site_bar',
            figure=fig1
        ),
